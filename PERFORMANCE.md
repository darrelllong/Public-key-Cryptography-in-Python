...
|---|---:|---:|---:|---:|
| RSA | 512 | 51.20 ms (±15%) | 65.08 ms *(±28%)* | 0.79× |
| RSA | 1024 | 374.79 ms *(±38%)* | 168.52 ms (±18%) | **2.22×** |
...
| Rabin | 2048 | 7.22 s (±1.1%) | 1.69 s *(±51%)* | **4.28×** |
| Paillier | 512 | 52.41 ms (±18%) | 63.66 ms (±23%) | 0.82× |
| Paillier | 1024 | 385.76 ms *(±27%)* | 166.00 ms (±17%) | **2.32×** |
| Paillier | 2048 | 2.81 s *(±31%)* | 892.87 ms (±10%) | **3.15×** |
| Schmidt-Samoa | 512 | 51.09 ms (±18%) | 62.33 ms *(±29%)* | 0.82× |
| Schmidt-Samoa | 1024 | 378.72 ms (±1.2%) | 163.99 ms (±17%) | **2.31×** |
| Schmidt-Samoa | 2048 | 2.81 s (±3.1%) | 871.01 ms (±3.9%) | **3.23×** |
| Cocks | 512 | 50.86 ms (±17%) | 63.89 ms *(±26%)* | 0.80× |
| Cocks | 1024 | 376.35 ms *(±39%)* | 165.74 ms (±15%) | **2.27×** |
| Cocks | 2048 | 2.79 s (±18%) | 1.73 s *(±200%)* | **1.61×** |

### Encrypt

| Algorithm | Bits | Python | Julia | Julia speedup |
|---|---:|---:|---:|---:|
| RSA | 512 | 13.8 µs (±2.3%) | 6.4 µs (±20%) | **2.14×** |
| RSA | 1024 | 27.2 µs (±0.5%) | 8.4 µs (±12%) | **3.26×** |
| RSA | 2048 | 87.4 µs (±0.7%) | 14.9 µs (±6.8%) | **5.86×** |
| ElGamal | 512 | 1.56 ms (±1.2%) | 706.3 µs *(±181%)* | **2.21×** |
| ElGamal | 1024 | 6.99 ms (±0.7%) | 2.65 ms *(±197%)* | **2.64×** |
| ElGamal | 2048 | 47.87 ms (±0.6%) | 7.56 ms *(±46%)* | **6.33×** |
| Rabin | 512 | 2.3 µs (±1.4%) | 1.2 µs (±9.9%) | **1.91×** |
| Rabin | 1024 | 4.7 µs (±0.5%) | 1.6 µs (±20%) | **2.92×** |
| Rabin | 2048 | 15.1 µs (±0.8%) | 3.8 µs (±15%) | **4.00×** |
| Paillier | 512 | 1.71 ms (±0.3%) | 408.2 µs (±1.9%) | **4.19×** |
| Paillier | 1024 | 11.63 ms (±0.3%) | 2.16 ms *(±90%)* | **5.39×** |
| Paillier | 2048 | 84.51 ms (±0.0%) | 10.59 ms *(±54%)* | **7.98×** |
| Schmidt-Samoa | 512 | 1.75 ms (±4.8%) | 844.1 µs *(±202%)* | **2.07×** |
| Schmidt-Samoa | 1024 | 11.91 ms (±8.1%) | 2.42 ms *(±85%)* | **4.92×** |
| Schmidt-Samoa | 2048 | 73.59 ms (±0.5%) | 11.31 ms *(±48%)* | **6.50×** |
| Cocks | 512 | 742.2 µs (±0.7%) | 631.0 µs *(±264%)* | **1.18×** |
| Cocks | 1024 | 3.22 ms (±0.6%) | 1.19 ms *(±154%)* | **2.71×** |
| Cocks | 2048 | 23.12 ms (±1.2%) | 5.85 ms *(±121%)* | **3.95×** |

### Decrypt

| Algorithm | Bits | Python | Julia | Julia speedup |
|---|---:|---:|---:|---:|
| RSA | 512 | 718.8 µs (±0.4%) | 634.1 µs *(±266%)* | **1.13×** |
| RSA | 1024 | 3.25 ms (±0.6%) | 1.01 ms *(±102%)* | **3.22×** |
| RSA | 2048 | 23.31 ms (±0.5%) | 5.00 ms *(±105%)* | **4.66×** |
| ElGamal | 512 | 773.6 µs (±0.2%) | 223.6 µs (±4.7%) | **3.46×** |
| ElGamal | 1024 | 3.59 ms (±0.2%) | 1.99 ms *(±268%)* | **1.81×** |
| ElGamal | 2048 | 23.34 ms (±0.2%) | 3.83 ms *(±77%)* | **6.09×** |
| Rabin | 512 | 359.1 µs (±0.9%) | 255.2 µs (±3.4%) | **1.41×** |
| Rabin | 1024 | 1.59 ms (±0.5%) | 1.04 ms *(±187%)* | **1.54×** |
| Rabin | 2048 | 6.93 ms (±0.2%) | 2.50 ms *(±72%)* | **2.77×** |
| Paillier | 512 | 1.63 ms (±0.3%) | 362.8 µs (±1.9%) | **4.49×** |
| Paillier | 1024 | 11.68 ms (±0.2%) | 2.19 ms *(±91%)* | **5.33×** |
| Paillier | 2048 | 83.45 ms (±0.1%) | 10.17 ms *(±50%)* | **8.21×** |
| Schmidt-Samoa | 512 | 761.0 µs (±0.2%) | 630.4 µs *(±263%)* | **1.21×** |
| Schmidt-Samoa | 1024 | 3.31 ms (±0.5%) | 1.17 ms *(±147%)* | **2.84×** |
| Schmidt-Samoa | 2048 | 22.86 ms (±0.1%) | 4.99 ms *(±104%)* | **4.58×** |
| Cocks | 512 | 134.2 µs (±1.6%) | 857.4 µs *(±343%)* | 0.16× |
| Cocks | 1024 | 790.9 µs (±1.9%) | 1.00 ms (±3.2%) | 0.79× |
| Cocks | 2048 | 3.46 ms (±0.3%) | 664.7 µs (±1.8%) | **5.20×** |
